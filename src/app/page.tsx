"use client"; // The widget cannot run in the server, the SDK can
...
export default function Home() {
  // Load the widget on React after the render
  useEffect(() => {
  ...
  const onPrompt = async () => {
    ...
    let acum = "";
    while (!done) {
      const { value, done: readerDone } = await responseStream.read();
      done = readerDone;
      if (value) {
        setLoading(false);
        console.log(value);
        //const chunk = decoder.decode(value, { stream: true });
        //console.log(chunk);  // Process the chunk here (e.g., append to UI)
        acum += JSON.parse(value).response;
        setReply(acum);
      }
    }
    setReply(acum);
  ...
  return (
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-8 row-start-2 items-center sm:items-start">
        ...
        {/* Usage for pure HTML websites
          <script src="path/to/offload.js"></script>
          <script>
            // Initialize the widget
            Offload.createWidget('widget-container', 'Hello from the widget!');
          </script>
        */}
